function createDonkeyKongGame(settings) {
    ...
    function generateLevel() {
        // Create slanted platforms (classic DK style)
        ...
        // Add ladders
        for (let i = 0; i < settings.ladderCount; i++) {
            const level = i + 1;
            const y = 470 - (level * levelHeight);
            const x = 150 + i * 100;
            ...
            game.ladders.push({ 
                x: x, 
                y: y - levelHeight + platformHeight, 
                width: 20, 
                height: levelHeight - platformHeight 
            });
        }
        
        // Add ladder from bottom to first level
        game.ladders.push({ x: 500, y: 390, width: 20, height: 80 });
        
        // Extra ladder to top
        game.ladders.push({ x: 300, y: 80, width: 20, height: 70 });
    }
    ...
    function checkWin() {
        ...
        if (game.player.x + game.player.width > game.princess.x &&
            game.player.x < game.princess.x + game.princess.width &&
    ...
    function nextLevel() {
        ...
        // Increase difficulty for next level - more frequent barrels, not faster
        settings.barrelFrequency = Math.max(40, settings.barrelFrequency - 15);
    }
    ...
    function render() {
        ...
        ctx.fillText(`Level: ${game.currentLevel}/${game.maxLevels}`, 10, 65);
